...
def order_home(request):
    ...
    towns = Town.objects.filter(isDeleted__exact=False).order_by('name')
    try:
    ...
    try:
        ...
        prev_attendance = EmployeeAttendance.objects.select_related().get(employeeID__staffIdIfAutogenerated__exact=user.pk,
                                                                          attendanceDate__icontains=prev_day, isDeleted__exact=False)
    except:
        emp = Employee.objects.get(staffIdIfAutogenerated__exact=user.pk,isDeleted__exact=False)
        prev_attendance = EmployeeAttendance()
        prev_attendance.employeeID_id = emp.pk
        prev_attendance.attendanceDate = prev_day
...
class OrderListByUserPerDayJson(BaseDatatableView):
    order_columns = ['id', 'partyName', 'orderTakenFrom','stockGroup','town', 'details', 'remark', 'datetime']
    ...
    def prepare_results(self, qs):
        ...
        for item in qs:
            ...
            json_data.append([
                ...
                escape(item.town),  # escape HTML for security reasons
...
class OrderListAdminJson(BaseDatatableView):
    order_columns = ['id', 'partyName', 'orderTakenFrom','town', 'details','location', 'orderTakenBy', 'datetime','remark']
    ...
    def filter_queryset(self, qs):
        ...
        if search:
            qs = qs.filter(
                ...
                Q(location__icontains=search)| Q(town__icontains=search)| Q(orderTakenBy__name__icontains=search)| Q(stockGroup__icontains=search)|
                Q(remark__icontains=search))
...
class OrderListManagerJson(BaseDatatableView):
    order_columns = ['id', 'partyName', 'orderTakenFrom', 'stockGroup','town','details','location', 'orderTakenBy', 'datetime','remark']
...
@check_group('Both')
def add_town_api(request):
    if request.method == 'POST':
        try:
            TownName = request.POST.get('TownName')

            obj = Town()
            obj.name = TownName
            obj.save()
            messages.success(request, 'Town name added successfully.')
            return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
class TownListJson(BaseDatatableView):
    order_columns = ['id','name',  'datetime']

    def get_initial_queryset(self):

        return Town.objects.select_related().filter(isDeleted__exact=False)
    ...
    def prepare_results(self, qs):
        ...
        for item in qs:
            if 'Moderator' in self.request.user.groups.values_list('name', flat=True):
                ...
                action = '''<span><a class="hideModerator" data-toggle="modal" data-target="#defaultModalEditTown" onclick="editTown('{}','{}')"><button style="background-color: #3F51B5;color: white;" type="button"
                               class="btn  waves-effect " data-toggle="modal"
                               data-target="#largeModalEdit">
                           <i class="material-icons">border_color</i></button> </a></span>'''.format(item.pk, item.name)
...
@check_group('Both')
def edit_town_api(request):
    if request.method == 'POST':
        try:
            ECIDT = request.POST.get('ECIDT')
            townEdit = request.POST.get('townEdit')

            obj = Town.objects.get(pk = int(ECIDT))
            obj.name = townEdit
            obj.save()
