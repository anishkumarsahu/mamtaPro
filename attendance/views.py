...
def genereate_attendence_report(request):
    ...
    employees = Employee.objects.filter(isDeleted=False, autogenerated__exact=False).order_by('name') if staff == 'all' else \
        Employee.objects.filter(isDeleted=False, pk=int(staff), autogenerated__exact=False).order_by('name')
