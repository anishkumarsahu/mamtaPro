...
from .utils import generate_emp_from_staff
...
class EmployeeListForAttendanceAdminBasicJson(BaseDatatableView):
    ...
    def get_initial_queryset(self):
        ...
        return Employee.objects.select_related().filter(isDeleted__exact=False, autogenerated__exact=False)
    ...
    def prepare_results(self, qs):
        ...
        for item in qs:
            ...
            if not attend.loginPhoto:
                ...
                loginPhoto = '''<img class="imageTable zoom" src="{}" alt="" >'''.format(
                    attend.loginPhoto.url),

            if not attend.logoutPhoto:
                logoutPhoto = 'N/A'
            else:
                logoutPhoto = '''<img class="imageTable zoom" src="{}" alt="" >'''.format(
                    attend.logoutPhoto.url),
...
def process_employee_attendance_batch(employees, month, days_in_month, attendance_dict):
    ...
    with ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(process_employee_attendance, emp, month, days_in_month, attendance_dict): emp
...
def genereate_attendence_report(request):
    ...
    startDate = datetime.strptime(sDate, '%d/%m/%Y').date()
    endDate = datetime.strptime(eDate, '%d/%m/%Y').date()
    ...
    months = OrderedDict(((startDate + timedelta(days=i)).strftime("%Y-%m"), None)
                         for i in range((endDate - startDate).days + 1)).keys()

    # Fetch employees
    if staff == 'all':
        employees = Employee.objects.filter(isDeleted=False, autogenerated=False).order_by('name')
    else:
        employees = Employee.objects.filter(isDeleted=False, pk=int(staff), autogenerated=False).order_by('name')

    paginator = Paginator(employees, 30)
    data = []

    # Process all months, but only add employees once (per month block)
    for month in months:
        month_start = datetime.strptime(month, '%Y-%m')
        days_in_month = calendar.monthrange(month_start.year, month_start.month)[1]
        month_end_date = month_start.replace(day=days_in_month).date()

        filter_start = max(startDate, month_start.date())
        filter_end = min(endDate, month_end_date)
        ...
        for page_number in paginator.page_range:
            ...
            attendance_records = EmployeeAttendance.objects.filter(
                attendanceDate__range=(filter_start, filter_end),
                employeeID__in=employee_ids
            ).select_related('employeeID')

            attendance_dict = defaultdict(lambda: None, {
                (rec.employeeID_id, rec.attendanceDate.strftime('%Y-%m-%d')): rec
                for rec in attendance_records
            })

            results = process_employee_attendance_batch(page.object_list, month, days_in_month, attendance_dict)
            data.extend(results)

    # Prepare days for the last processed month (for table headers)
    last_month = list(months)[-1]
    last_month_days = calendar.monthrange(int(last_month[:4]), int(last_month[5:]))[1]
    ...
    context = {
        ...
        'report': 1,
        'today': today,
        'days': range(1, last_month_days + 1),
    ...
    HTML(string=html).write_pdf(response, stylesheets=[
        CSS(string='@page { size: A4 landscape; margin: .3cm ; }')
    ])

    return response

def generate_attendance_for_order_emp_report(request):
    ...
    employees = Employee.objects.filter(isDeleted=False, autogenerated__exact=True).order_by('name') if staff == 'all' else \
        Employee.objects.filter(isDeleted=False, pk=int(staff),autogenerated__exact=True).order_by('name')
...
class OrderEmployeeListForAttendanceAdminBasicJson(BaseDatatableView):
    order_columns = ['id', 'photo', 'name', 'loginTime', 'loginPhoto', 'logoutTime','logoutPhoto', 'inLocation', 'outLocation']

    def get_initial_queryset(self):

        return Employee.objects.select_related().filter(isDeleted__exact=False, autogenerated__exact=True)
...
@csrf_exempt
def order_employee_attendance_post_api(request):
    if request.method == 'POST':
        try:
            loginEmpID = request.POST.get('loginEmpID')
